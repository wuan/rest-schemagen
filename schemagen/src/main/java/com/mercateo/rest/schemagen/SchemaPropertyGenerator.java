...
import com.mercateo.rest.schemagen.generator.PathContext;
import com.mercateo.rest.schemagen.generator.ReferencedJsonPropertyFinder;
...
public class SchemaPropertyGenerator {
    ...
    private static final Map<Class<?>, JsonProperty> builtins = ImmutableMap.of( //
            Date.class, JsonProperty.builderFor(Integer.class).withName("n/a").withIsRequired(true).build(), URL.class,
            JsonProperty.builderFor(String.class).withName("n/a").build());
     ...
     * given {@link com.mercateo.rest.schemagen.generator.ObjectContextBuilder}
     ...
     * @return property hierarchy
     */
    public JsonPropertyResult generateSchemaProperty(ObjectContext<?> objectContext, SchemaPropertyContext context) {
        ...
        return ImmutableJsonPropertyResult.of(rootJsonProperty, referencedJsonPropertyFinder
            .getReferencedJsonProperties(rootJsonProperty));
    }
    ...
    private List<JsonProperty> getProperties(final ObjectContext<?> objectContext, final PathContext pathContext,
            final SchemaPropertyContext context) {
        ...
        return sortedFields.map(field -> {
            ...
            if (!fieldNames.contains(name)) {
                ...
                throw new IllegalStateException("field name <" + name + "> collision in class " + objectContext
                    .getRawType()
                    .getSimpleName());
            }
            return determineProperty(field.getName(), fieldContextMap.get(field).forField(field), pathContext, context);
        }).collect(Collectors.toList());
    }

    private Map<Field, ObjectContext> getFieldContextMap(ObjectContext objectContext, SchemaPropertyContext context) {
    ...
    private void addFieldToMap(Field field, ObjectContext objectContext, Map<Field, ObjectContext> fieldContextMap,
            Set<Type> unwrappedTypes, SchemaPropertyContext context) {
        if (objectContext.isApplicable(field, context)) {
            if (field.getAnnotation(JsonUnwrapped.class) != null) {
                fieldContextMap.putAll(getUnwrappedFieldsMap(field, objectContext, unwrappedTypes, context));
            } else {
    ...
    private Map<Field, ObjectContext> getUnwrappedFieldsMap(Field field, ObjectContext objectContext,
            Set<Type> unwrappedTypes, SchemaPropertyContext context) {
        ...
        if (unwrappedTypes.contains(unwrappedType)) {
            throw new IllegalStateException(String.format("recursion detected while unwrapping field <%s> in <%s>",
                    field.getName(), unwrappedType.getTypeName()));
        }
    ...
    private JsonProperty determineObjectProperty(String name, ObjectContext<?> objectContext, PathContext pathContext,
            SchemaPropertyContext context) {
        ...
        if (pathContext.isKnown(type)) {
            return builder
                .withPath(pathContext.getCurrentPath() + "/" + name)
                .withRef(pathContext.getPath(type))
                .build();
        }

        final PropertyType propertyType = objectContext.getPropertyType();
        pathContext = pathContext.enter(name, propertyType == PropertyType.OBJECT ? type : null);
        return builder
            .withPath(pathContext.getCurrentPath())
            .withProperties(getNestedProperties(objectContext, pathContext, context))
            .build();
    }

    private List<JsonProperty> getNestedProperties(ObjectContext<?> objectContext, PathContext pathContext,
            SchemaPropertyContext context) {
    ...
    private List<JsonProperty> getDictProperties(ObjectContext<?> objectContext, PathContext pathContext,
        ...
        return Stream.of(keyType.getEnumConstants()).map(enumConstant -> {
