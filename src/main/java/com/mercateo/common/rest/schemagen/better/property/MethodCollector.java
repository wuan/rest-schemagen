...
public class MethodCollector implements RawPropertyCollector {
    ...
    public Stream<RawProperty> forType(GenericType<?> genericType) {
        return Arrays.stream(genericType.getRawType().getDeclaredMethods()).filter(method -> !method
                .isSynthetic()).filter(method -> method.getDeclaringClass() != Object.class).filter(
                        method -> method.getReturnType() != void.class).filter(method -> method
                                .getParameterCount() == 0).map(method -> mapRawDataProperty(method,
                                        genericType.getType()));
    }
